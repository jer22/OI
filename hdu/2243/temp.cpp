...
int num;
char str[10];
LL mod=10330176681277348905ull;
...
LL solve(Mat a,int k)
{
    ...
    int i,j;
    b.init();
    for(i=0;i<len;i++)  //左上部分
        for(j=0;j<len;j++)
            b.a[i][j]=a.a[i][j];
    for(i=0;i<len;i++)  //右上部分
    {
        for(j=len;j<2*len;j++)
            b.a[i][j]=(i==j-len);
    }
    for(i=len;i<2*len;i++) //右下部分
    {
        for(j=len;j<2*len;j++)
            b.a[i][j]=(i==j);
    }
...
int main()
{
    ...
    while(scanf("%d%d",&n,&m)==2)
    {
        ...
        len=id;
        LL all=sum(26,m); //26+26^1+26^2+....+26^m
        // cout << m << endl;
        LL left=solve(res,m); //A+A^1+A^2+....+A^m
        all-=left;
        if(all<0)
            all+=mod;
        printf("%llu\n",all);
