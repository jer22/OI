...
const int PN = 159, PL = 59, TN = 159, TL = 1009;
char P[PN][PL], T[TL];
int li[PN], hi[PN], pn, tn;
int profit; VI match;

namespace ACM{
    const int N = PN*PL+9, Z = 26;
    int trans[N][Z], fail[N], cnt[N], id[N], Q[N], cz, op, tt;

    int new_node(){
        RST(trans[tt]), fail[tt] = cnt[tt] = id[tt] = 0;
        return tt++;
    }
...
#define c (*cur - 'a')
    ...
    void Insert(char str[], int idd){
        ...
        id[u] = idd, cnt[u] += li[idd];
    }

#define vis Q
    void Run(char str[]){
        profit = 0, CLR(match); int t, u = 0; RST(vis); REP_S(cur, str){
            for (profit += cnt[t=u=v];t&&!vis[t];t=fail[t]){
...
#define f cap[i^1]
    ...
    int Solve(){
        ...
        RD(pn, tn), s = 0, t = pn + tn + 1, n = t + 1, m = 2, fill(hd, hd+n, 0);
        total = 0, ACM::Init();

        REP_1(i, pn){
            RD(li[i], hi[i]), ACM::Insert(RS(P[i]), i);
            add_edge(tn+i, t, hi[i]);
        }

        ACM::Build();

        REP_1(i, tn){
            RS(T); ACM::Run(T); total += profit, add_edge(s, i, profit);
            ECH(it, match) add_edge(i, tn+*it, INF);
            //cout << profit << " " << SZ(match) << endl;
        }
